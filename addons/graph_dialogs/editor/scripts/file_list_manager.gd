...
## File List Manager
##
## This script manages the file list in the Graph Dialogs editor.
...
## Character icon
var _char_icon := preload("res://addons/graph_dialogs/icons/character.svg")
...
func get_item_metadata(index: int) -> Dictionary:
	if index < 0 or index >= _file_list.item_count:
		printerr("[Graph Dialogs] Invalid file index to get metadata.")
		return {}
...
func set_item_metadata(index: int, metadata: Dictionary) -> void:
	if index < 0 or index >= _file_list.item_count:
		printerr("[Graph Dialogs] Invalid file index to set metadata.")
		return
...
func close_file(index: int = _current_file_index) -> void:
	...
	_dialogs_count -= 1 if metadata.data is GraphDialogsDialogueData else 0
	_characters_count -= 1 if metadata.data is GraphDialogsCharacterData else 0
...
func _switch_to_file(index: int) -> void:
	if index < 0 or index >= _file_list.item_count:
		printerr("[Graph Dialogs] Invalid file index to switch to.")
		return
