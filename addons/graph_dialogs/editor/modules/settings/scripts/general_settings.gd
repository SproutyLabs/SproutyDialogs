...
## General Settings
##
## This script handles the general settings panel in the Graph Dialogs editor.
## It allows to configure input actions, default scenes and canvas layers.
# -----------------------------------------------------------------------------

## Continue input action field
@onready var _continue_input_action_field: GraphDialogsComboBox = %ContinueInputActionField
## Default dialog box scene field
@onready var _default_dialog_box_field: GraphDialogsFileField = %DefaultDialogBoxField
## Default portrait scene field
@onready var _default_potrait_scene_field: GraphDialogsFileField = %DefaultPortraitSceneField
...
func _load_settings() -> void:
	...
	if GraphDialogsSettings.get_setting("default_dialog_box") == -1:
		_default_dialog_box_field.set_value("")
	else:
		_default_dialog_box_field.set_value(ResourceUID.get_id_path(
				GraphDialogsSettings.get_setting("default_dialog_box")
			)
		)
	if GraphDialogsSettings.get_setting("default_portrait_scene") == -1:
		_default_potrait_scene_field.set_value("")
	else:
		_default_potrait_scene_field.set_value(ResourceUID.get_id_path(
				GraphDialogsSettings.get_setting("default_portrait_scene")
			)
		)
	_dialog_box_canvas_layer_field.value = \
		GraphDialogsSettings.get_setting("dialog_box_canvas_layer")
	_portrait_canvas_layer_field.value = \
		GraphDialogsSettings.get_setting("portraits_canvas_layer")


## Handle when the continue input action is changed
func _on_continue_input_action_changed(new_value: String) -> void:
	GraphDialogsSettings.set_setting("continue_input_action", new_value)
...
func _on_dialog_box_canvas_layer_changed(new_value: int) -> void:
	GraphDialogsSettings.set_setting("dialog_box_canvas_layer", new_value)


## Handle when the portrait canvas layer is changed
func _on_portrait_canvas_layer_changed(new_value: int) -> void:
	GraphDialogsSettings.set_setting("portraits_canvas_layer", new_value)