...
signal options_processed(options: Array, next_nodes: Array, option_keys: Array, striked_flags: Array)
...
func _process_options(node_data: Dictionary) -> void:
	...
	for i in range(node_data.options_keys.size()):
		...
		if not condition_data.is_empty() and condition_data.get("enabled", false):
			...
			var visibility = int(condition_data.get("visibility", 0)) # 0 hidden, 1 striked
		...
		if show_option:
			...
			striked_flags.append(strike_option)

	options_processed.emit(filtered_options, filtered_next_nodes, filtered_option_keys, striked_flags)
